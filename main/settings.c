...
void settings_init(void) {
    ...
    if (ret == ESP_OK) {
        ...
        nvs_get_i32(handle, "interval", &interval);
        
        len = sizeof(mqtt_broker);
        nvs_get_str(handle, "mqtt_broker", mqtt_broker, &len);
        
        nvs_get_i32(handle, "mqtt_port", &mqtt_port);
    ...
    ESP_LOGI(TAG, "Interval: %d sec, MQTT: %s:%d", interval, mqtt_broker, mqtt_port);
}
...
esp_err_t settings_save(void) {
    ...
    nvs_set_i32(handle, "interval", interval);
    nvs_set_str(handle, "mqtt_broker", mqtt_broker);
    nvs_set_i32(handle, "mqtt_port", mqtt_port);
